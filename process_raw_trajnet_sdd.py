...
def collect_data(set_name, dataset_type = 'image', batch_size=512, time_thresh=48, dist_tresh=100, scene=None, verbose=True, root_path="./"):
    ...
    rel_path = f'data/trajnet_sdd_actual/{set_name}'
    ...
    file_whole = root_path + rel_path + part_file

    print(f"file_whole: {file_whole}")
    for file in glob.glob(file_whole):
        scene_name = file[len(root_path+rel_path)+1:-4]# + file[-5]
        ...
        # extend seq names by the number of people in the scene
        full_seq_names.extend([scene_name for _ in range(len(list(data_by_id.keys())))])
    ...
    print(f"full_dataset.shape: {len(full_dataset)}, {len(full_dataset[0])}", len(full_dataset[0][0]), len(full_dataset[0][0][0], ))
    print(f"all shapes of full_masks: {[a.shape[0] for a in full_masks]}")
    print(f"len(seq_names):", len(full_seq_names))
    return full_dataset, full_masks, full_seq_names

def generate_pooled_data(b_size, t_tresh, d_tresh, train=True, scene=None, verbose=True, root_path=None):
    if train:
        full_train, full_masks_train, seq_names_train = collect_data("train", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh, scene=scene, verbose=verbose, root_path=root_path)
        train = [full_train, full_masks_train, seq_names_train]
        train_name = "data/train_{0}_{1}_{2}_{3}.pickle".format('all' if scene is None else scene[:-2] + scene[-1], b_size, t_tresh, d_tresh)
        with open(train_name, 'wb') as f:
            pickle.dump(train, f)

    if not train:
        full_test, full_masks_test, seq_names_test = collect_data("test", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh, scene=scene, verbose=verbose, root_path=root_path)
        test = [full_test, full_masks_test, seq_names_test]
        test_name = "data/test_{0}_{1}_{2}_{3}.pickle".format('all' if scene is None else scene[:-2] + scene[-1], b_size, t_tresh, d_tresh)# + str(b_size) + "_" + str(t_tresh) + "_" + str(d_tresh) + ".pickle"
...
generate_pooled_data(4096,0,100, train=False, verbose=True, root_path="./")
generate_pooled_data(512,0,100, train=True, verbose=True, root_path="./")
